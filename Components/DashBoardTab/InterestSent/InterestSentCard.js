...
const InterestSentCardSkeleton = () => {
...
export const InterestSentCard = ({ sortBy = "datetime" }) => {
    ...
    if (success) {
      ...
      setProfiles((prevProfiles) =>
        prevProfiles.map((profile) =>
          profile.myint_profileid === profileId
            ? { ...profile, myint_profile_wishlist: newStatus === "1" ? 1 : 0 }
            : profile
        )
      );
    } else {
  ...
  return (
    <View style={styles.profileScrollView}>
      <FlatList
        data={profiles}
        keyExtractor={(item) => String(item.myint_profileid)}
        ...
        renderItem={({ item: profile }) => {
          ...
          const isSaved = bookmarkedProfiles.has(profile.myint_profileid);
          const rawImage = Array.isArray(profile.myint_Profile_img)
            ? profile.myint_Profile_img[0]
            : profile.myint_Profile_img;

          const matchScore =
            profile.myint_match_score ??
            profile.matching_score ??
            profile.matchScore ??
            0;

          const ageHeightText = `${profile.myint_profileid || "N/A"} · ${
            profile.myint_profile_age || "N/A"
          } yrs · ${profile.myint_height?.height_desc || "N/A"}`;

          // Formatted Profession Text with Degree Fallback
          const professionText =
            [profile.myint_degree, profile.myint_profession]
          ...
          // Location extraction
          const locationText = profile.myint_city || profile.myint_location;

          return (
            <TouchableOpacity
              style={styles.card}
              onPress={() =>
                !isMarried && handleProfileClick(profile.myint_profileid)
              }
            ...
            >
                  ...
                  {profile.myint_star ? (
                    <View style={styles.tagsRow}>
                      <View style={styles.tag}>
                        <Text style={styles.tagText}>{profile.myint_star}</Text>
                      </View>
                    </View>
                  ) : null}
                </View>
              ...
              <View style={styles.cardFooter}>
                <Text style={styles.lastActiveText}>
                  Expressed Interest on {profile.myint_lastvisit || "N/A"}
                </Text>
                ...
                <View style={styles.btnGroup}>
                  {!isMarried && (
                    <TouchableOpacity
                      onPress={(e) => {
                        e?.stopPropagation?.();
                        handleSavePress(profile.myint_profileid);
                      }}
                      style={[
                        styles.shortlistBtn,
